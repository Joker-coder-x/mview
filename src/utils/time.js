...
//将毫秒转换成Unix时间戳
function toUnixTimestamp(millisecond){return Math.floor(millisecond/1000);}
//将unix时间戳转换成毫秒
function toMillisecond(timestamp){return timestamp*1000;}
...
const Time={
    ...
    getTipTime(timestamp,isUnix=false){
        ...
        if(timer<60){
           ...
           tip=`${((new Date()).getMonth()-(new Date(this.toMillisecond(timestamp))).getMonth())}月前`;
        }else{
