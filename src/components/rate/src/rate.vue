    ...
    <div class="m-rate">
        <span v-for="item in count"
            ...
            <template v-if="!showCharacter">
                ...
                <span :class="[getIcon,'star-after']" :style="getStarAfterStyle(item)"></span>
            </template>
            <template v-else>
                <span>{{character}}</span>
                <span class="star-after" :style="getStarAfterStyle(item)">{{character}}</span>
            </template>
        </span>
        <span v-if="showScore" 
             :style="{color:textColor,marginLeft:textSpacing+'px'}"><slot>{{currentValue }}</slot></span>
        <span v-else-if="showText&& texts.length>0" 
              :style="{color:textColor,marginLeft:textSpacing+'px'}">{{lastActiveIndex>0?texts[lastActiveIndex-1]:texts[0]}} </span>
    </div>
</template>
        ...
        getStarAfterStyle(item){
            ...
            if(this.activeIndex>=0){
                if(item<this.activeIndex){
                    ...
                    width=this.float||this.allowHalf?`${this.lastItemWidthRate*100}%`:'100%';
                }
            }else{
                if(item<= parseFloat(currentValue)||item<lastActiveIndex){
                    width='100%';
                }else{
                    if(item>currentValue&&currentValue.toString().indexOf('.')>-1&&item-currentValue<1){
