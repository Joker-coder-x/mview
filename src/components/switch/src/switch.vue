<template>
    <div class="m-switch" 
        ...
        @click.capture="handleChange">
         <label  class="m-switch__lable" 
                 v-if="showInactiveText&&!inlineText"
                 :style="getSwitchInactiveTextStyle"
                :class="[disabled?'is-disabled':'']">{{inactiveText}}</label>
        <div class="m-switch__inner">
            <input type="checkbox" 
               ...
               :disabled="disabled">
            <div class="m-switch-warp" 
                :class="[disabled?'is-disabled':'']"
                :style="getSwitchWarpStyle">
                <div class="m-switch__slider"></div>
                <div class="m-switch__bg" :style="getSwitchBgStyle"></div>
				<span class="inline-text-inactive" v-if="inlineText&&showInactiveText&&currentValue">{{inactiveText}}</span>
				<span class="inline-text-active"   v-if="inlineText&&showActiveText&&!currentValue">{{activeText}}</span>
            </div>
        </div>
        <label class="m-switch__lable" 
               v-if="showActiveText&&!inlineText"
               :style="getSwitchActiveTextStyle"
              :class="[disabled?'is-disabled':'']">{{activeText}}</label>
    </div>
</template>
...
export default {
    ...
    data(){
        return {
			...
			innerDisabled:false,
			moreLightActiveColor:lightenDarkenColor(this.activeColor,MORE_LIGHT_NUM),
			moreLightInactiveColor:lightenDarkenColor(this.inactiveColor,MORE_LIGHT_NUM),
			openId:null
        }
    },
    ...
    computed:{
        ...
        getSwitchActiveTextStyle(){
            return {
                color:this.currentValue===true?(this.activeTextColor||this.defaultTextColor):this.defaultTextColor,
        ...
        getSwitchInactiveTextStyle(){
            return {
                color:this.currentValue===false?(this.inactiveTextColor||this.defaultTextColor):this.defaultTextColor,
    ...
    methods:{
		changeOutlineColor(){
			const color=this.currentValue?this.moreLightActiveColor:this.moreLightInactiveColor;
	...
	.m-switch{
		...
		.m-switch__inner{
			...
			.m-switch-warp{
				...
				.inline-text-inactive,.inline-text-active{
					position: absolute;
					font-size: .7em;
					line-height: .7em;
					top: 50%;
