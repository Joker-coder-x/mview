<template>
    <div class="m-verification-common" v-if="!isMask">
        <canvas :id="canvasId"
    ...
    <div class="m-verification-common" v-else>
...
export default {
    ...
    computed:{
        getCanvasStyle(){
            return {
                border:this.isBorder?`${this.borderWidth} solid ${this.borderColor}`:''
            }
        }
    },
    ...
    methods:{
        draw(){
            ...
            this.__veriThis__.draw(this.isUseDefaultConfig?this.defaultConfig:JSON.parse(JSON.stringify(this.config)));
        },
    ...
    mounted(){
        ...
        if(this.customRefreshTargetSelector){
            ...
            const refreshTarget =   document.querySelector(this.customRefreshTargetSelector);
       ...
       if(this.customSubmitTargetSelector){
           const submitTarget = document.querySelector(this.customSubmitTargetSelector);
    ...
    beforeDestroy(){
        ...
        if(this.__refreshTarget__){
            this.__refreshTarget__.removeEventListener('click',this.__refreshTargetHandler__);
        ...
        if(this.__inputTarget__){
            this.__inputTarget__.removeEventListener('input',this.__inputTargetHandler__);
        ...
        if(this.__submitTarget__){
            this.__submitTarget__.removeEventListener('click',this.__submitTargetHandler__);
...
<style scoped>
.m-verification-common{
    display: inline-block;
}

.m-verification-common-canvas{
    display: inline-block;
}
