...
<script>
import { aiMove, easeout } from "./utils.js";
  ...
  methods: {
    ...
    getContentChild() {
      return this.$children.filter(
        c => c.$options.name === "MSlotsItemContent"
      );
    },
        ...
        aiMove(
          ...
          () => {
            ...
            if (vm.useTimes < vm.times) {
              ...
              //换位
              completeList.push(
                this.findName(content.children[content.children.length - 1])
              );
          ...
          "bottom",
          vm.slotsRef.easeout
            ? easeout(vm.useTimes, vm.slotsRef.speed, vm.times)
            : vm.slotsRef.speed
        )(); //aiMove end
      }; // handler end
