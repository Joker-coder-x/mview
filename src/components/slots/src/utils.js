...
export function aiMove(el, distance, callback, direction = 'bottom', moveRange = 2) {
    ...
    return function move() {
        if (distance > 0) {
            ...
            el.style[direction] = `${direction==='bottom'||direction==='left'?'-':''}${Math.round(Math.abs(lastOffset))}px`;
