<template>
    <div class="warp">
            <canvas ref="canvas" class="main-canvas" :width="width" :height="height"></canvas>
            <canvas ref="clip" class="clip-canvas" width="100" :height="height" ></canvas>
            <span class="mIcon-spinner11 refresh" @click="handleReset"></span>
        <div class="slider-warp" 
                ref="sliderWarp"
                :style="getSliderWarpStyle">
            <div :class="[
                    'slider unselect',
                    ...
                    v-show="useErrorTimes!==maxErrorTimes"></div>
            <div class="slider-background" :style="getSliderBackgroundStyle" ></div>
            <span class="slider-warp-plhr" v-show="useErrorTimes!==maxErrorTimes">{{placeholder}}</span>
            <span class="slider-warp-error-msg" @click="handleReDo" v-show="useErrorTimes===maxErrorTimes"><span class="mIcon-cross-circle-fill" ></span> 失败次数过多，点击重试</span>
        </div>
    </div>
</template>
...
export default {
    ...
    watch:{
        config(newVal){
            vm.defaultConfig=mergeOptions(vm.defaultConfig,newVal,['canvas','clipCvs']);
        },
    },  
    ...
    mounted(){
        ...
        vm.defaultConfig = mergeOptions(vm.defaultConfig,vm.config,['canvas','clipCvs']);
        ...
        const handleOnLoad=function(veriThis){
    ...
    beforeDestroy(){
        ...
        if(vm.__hanldeMouseDown__){
            this.$refs.slider.removeEventListener('mousedown',vm.__hanldeMouseDown__);
...
<style scoped>
        ...
        .slider-warp-plhr,.slider-warp-error-msg{
            position: absolute;
            left: 50%;
            top: 50%;
