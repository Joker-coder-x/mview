...
const VerificationSlider = (() => {
  ...
  function genrate(
    ...
    const data = context.getImageData(
      infoObj.clipX - borderWidth,
      infoObj.clipY - borderWidth,
      infoObj.clipWidth + borderWidth * 2,
      infoObj.clipHieght + borderWidth * 2
    );
     ...
     * @returns {Boolean}
     * **/
    const verification = (deviationRange = 10) =>
      Math.abs(clipCvs.offsetLeft - clipPosition[0]) <= deviationRange ?
      true :
      false;
     ...
     * **/
    const setClipCvsOffsetLeft = offsetLeft =>
      (clipCvs.style.left = `${offsetLeft}px`);
  ...
  function initImageList(imgList, callback) {
    ...
    new Promise(resolve => {
      ...
      () => callback && callback(readyImageList),
   ...
   * @returns {{x,y}}
   */
  function randomGenrateClipArea(
    cvsWidth,
    cvsHeight,
    clipAreaWidth,
    clipAreaHeight
  ...
  class VerificationSlider {
    ...
    //生成裁剪区域信息
    genrateClipPosition() {
      const randomPos = randomGenrateClipArea(
        this.canvas.width,
        this.canvas.height,
        this.clipAreaWidth,
        this.clipAreaHeight
    ...
    genrateVerificationArea() {
      this.verifier = genrate(
        this.canvas,
        this.clipCvs,
        this.getCurImg(),
        this.genrateClipPosition(),
        this.borderWidth,
        this.borderColor
      );
