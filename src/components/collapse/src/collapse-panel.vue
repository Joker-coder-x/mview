    ...
    <div class="m-collapse-panel" @click="handlePanelClick">
        <div class="header" @click="handleOpen">
            <m-icon :name="iconName" 
                    ...
                    v-if="!hideArrow&&arrowPosition==='front'"></m-icon>
            <slot></slot>
            <m-icon :name="iconName" class="icon" v-if="!hideArrow&&arrowPosition==='behind'"></m-icon>
        </div>
...
export default {
    ...
    props:{
        ...
        arrowPosition:{
            type:String,
            default:arrowPositionList[0],
            validator(value){
                return arrowPositionList.indexOf(value)!==-1;
            }
        },
        name:{
            type:String|Number,
...
.fold-enter-active{
    animation:  fold-vertical-in .05s reverse;
}

.fold-leave-active{
    animation:  fold-vertical-in .05s;
}
</style>